% A script for preliminary sizing of the multi-modal vehicle's power systems
% Author: Anirudh Gundimeda
% Institution: University of Bristol
% Purpose: MSc Dissertation
% Project Title: Multi-modal Robotic Design for Conservation Applications

% In this script, power model was developed for multi-rotor,
% fixed-wing and driving operations individually and are used as required
% in multiple example paths.
...
FW.cruise_lift_coefficient_ = 0.7065; % [-] cruise lift coefficient
...
MR.cruise_speed_ = 10; % [m/s] Multi-rotor cruise speed
...
for distance = 0:1:1000
    ...
    FW.cruise_distance_ = mission.distance_*0.9;
    DM.distance_ = mission.distance_*0.1;
    ...
    % Joules
    mission.energy_required_multimodal = ((MR.drop_current_draw_)+DM.total_ampere_draw_+FW.cruise_current_required+(MR.climb_current_draw_))*motor.reference_voltage_;
    
    
    %%%%% Multi-rotor operation and energy consumption
    MR.cruise_distance_ = mission.distance_;
    
    % Adding the current drawn by bmulti-rotor based on how many times 
